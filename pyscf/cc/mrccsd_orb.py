...
def expT(T1, T2, nfc, nfv, power):
    ...
    Na = cistring.num_strings(nmoa,nea)
    Nb = cistring.num_strings(nmob,neb)

    addr2str = cistring.addr2str
    str2addr = cistring.str2addr
    cre_des_sign = cistring.cre_des_sign
    def imt(N,nmo,no,nv,nfc,t1,t2):
        ne = no+nfc
        ...
        return S, T
    t = time.time()
    Sa, Taa = imt(Na,nmoa,noa,nva,nfc[0],t1a,t2aa)
    Sb, Tbb = imt(Nb,nmob,nob,nvb,nfc[1],t1b,t2bb)
    ...
    for n in range(1,power+1):
        ci = T(ci)
        ci_ += ci/math.factorial(n)
    return ci_

def expK(fa, fb, nea, neb, power):
    nmoa = fa.shape[1]
    nmob = fb.shape[1]
    Na = cistring.num_strings(nmoa,nea)
    Nb = cistring.num_strings(nmob,neb)
    Fa = np.linalg.inv(fa[:nea,:nea])
    Fb = np.linalg.inv(fb[:neb,:neb])
    ca = np.dot(Fa,fa[:nea,nea:])
    cb = np.dot(Fb,fb[:neb,neb:])
                ...
                if strI & h != 0:
                    for a in range(nv):
                        ...
                        if strI & p == 0:
                            str1 = strI ^ h | p
                            K = str2addr(nmo,ne,str1)
                            sgn = cre_des_sign(cre,des,strI)
                            C[K,I] += c[i,a]*sgn
        return C
    t = time.time()
    Ca = imt(Na,nmoa,nea,ca)
    Cb = imt(Nb,nmob,neb,cb)
    print('tensor construction: {}'.format(time.time()-t))

    def K(ci):
        ci_  = np.dot(Ca,ci)
        ci_ += np.dot(Cb,ci.T).T
        return ci_
...
def _hop(moa, mob, hcore, eri, ne):
    ...
    h2e = direct_uhf.absorb_h1e(h1e, eri, nmoa, ne, .5) 
    def hop(c): 
        return direct_uhf.contract_2e(h2e, c, nmoa, ne)
    return hop

def _ovlp(nmoa,nmob,nea,neb,fa,fb):
    occslsta = cistring._gen_occslst(range(nmoa),nea)
    occslstb = cistring._gen_occslst(range(nmob),neb)
    Na = occslsta.shape[0]
    Nb = occslstb.shape[0]
    ovlp = np.zeros((Na,Nb))
    for I in range(Na):
        ...
        for J in range(Nb):
            occJ = occslstb[J,:]
            fb_ = fb[:,occJ] 
            ovlp[I,J] = np.linalg.det(fa_[:nea,:])*np.linalg.det(fb_[:neb,:])
    return ovlp
...
# FIXME: write method to etract # frozen core/virtual from input frozen
# currently done by explicitly providing nfc, nfv
    def __init__(self, mf, moa, mob, e_mf=None, power=None, 
        frozen=0, nfc=(0,0), nfv=(0,0), mo_coeff=None, mo_occ=None):
        ccsd.CCSD.__init__(self, mf, frozen, mo_coeff, mo_occ)

        # unpack dimensions
        power = sum(mf.mol.nelec) if power is None else power
        self._power = power
        self._nfc = nfc
        self._nfv = nfv 
        ...
        Na = cistring.num_strings(nmoa, nea)
        Nb = cistring.num_strings(nmob, neb)
        self._ket = np.empty((nref, Na, Nb))
        self._bra = np.empty((nref, Na, Nb))
        self._etot = None 
        self._ci = None
    ...
    def kernel_t(self, t1=None, t2=None, eris=None, mbpt2=False):
        N, _, _ = self._ket.shape
        _, nmo, _ = self._moa.shape
        for I in range(N):
            print('\nCCSD for mf {}'.format(I))
            self.mo_coeff = self._moa[I,:,:], self._mob[I,:,:]
            eris = self.ao2mo(self.mo_coeff)
            ...
            mo_eb_o = eris.mo_energy[1][:noa]
            mo_eb_v = eris.mo_energy[1][noa:]
            print('CCSD mo energies (oa, va):\n{}, {}'.format(mo_ea_o, mo_ea_v))
            print('CCSD mo energies (ob, vb):\n{}, {}'.format(mo_eb_o, mo_eb_v))

            # reference energy can be passed, deal with later
            self._scf.e_tot = self.ref_energy() + self._scf.mol.energy_nuc()
            print('reference energy: {}'.format(self._scf.e_tot))
#            Ecorr, t1, t2 = self.ccsd(t1, t2, eris, mbpt2)
#            t = time.time() 
#            self._ket[I,:,:] = expT(t1,t2,self._nfc,self._nfv,self._power)
#            print('ket formation time: {}'.format(time.time()-t))
#            print('coeffs: {}, {}'.format(self._ket[I,0,0],self._ket[I,1,1]))
#            print('t1a norm: {}'.format(np.linalg.norm(t1[0])/nmo))
#            print('t1b norm: {}'.format(np.linalg.norm(t1[1])/nmo))
#            print('t2aa norm: {}'.format(np.linalg.norm(t2[0])/nmo))
#            print('t2ab norm: {}'.format(np.linalg.norm(t2[1])/nmo))
#            print('t2bb norm: {}'.format(np.linalg.norm(t2[2])/nmo))
        return 
    ...
    def kernel_ci(self):
        ...
        eri = self._scf._eri
        nea, neb = self._scf.mol.nelec
        for I in range(N):
             moa = self._moa[I,:,:]
             mob = self._mob[I,:,:]
             hop = _hop(moa,mob,hcore,eri,(nea,neb))
             ket = self._ket[I,:,:]
             hket = hop(ket)
             for J in range(N):
                 moa_ = self._moa[J,:,:]
                 mob_ = self._mob[J,:,:]
                 fa = np.linalg.multi_dot([moa_.T,ovlp,moa]) 
                 fb = np.linalg.multi_dot([mob_.T,ovlp,mob]) 
                 detf = np.linalg.det(fa[:nea,:nea])*np.linalg.det(fb[:neb,:neb])
                 print('I,J: {},{}'.format(I,J))
                 print('ovlp: {}'.format(detf)) 
                 t = time.time()
                 if detf > 1e-3:
                     bra = expK(fa,fb,nea,neb,self._power)
                     bra *= detf
                 else:
                     nmoa = moa.shape[1]
                     nmob = mob.shape[1]
                     bra = _ovlp(nmoa,nmob,nea,neb,fa,fb)
                 print('bra formation time: {}'.format(time.time()-t))
                 H[J,I] = einsum('JI,JI',bra,hket)
                 S[J,I] = einsum('JI,JI',bra,ket)
#
#                 nmoa = moa.shape[1]
#                 nmob = mob.shape[1]
#                 ovlp_ = _ovlp(nmoa,nmob,nea,neb,fa,fb)
#                 h = einsum('JI,JI',ovlp_,hket)
#                 s = einsum('JI,JI',ovlp_,ket)
#                 print('ovlp error: {}'.format(h/s-H[J,I]/S[J,I]))
#                 print('ovlp error: {}'.format(np.linalg.norm(bra-ovlp_)))
#        exit()
        # solve for CI coefficient
        print(H)
        print(S)
        print('diagonal: {}'.format(np.divide(np.diagonal(H),np.diagonal(S))
                                    + self._scf.mol.energy_nuc()))
        w, v = scipy.linalg.eig(H, S)
        self._ci = v
        self._etot = w.real + self._scf.mol.energy_nuc()
        print('MRCCSD energies: {}'.format(self._etot))
        print('Imaginary parts: {}'.format(w.imag))
        print('MRCCSD CI vecs:\n{}'.format(v.real))
    ...
    def ref_energy(self):
        ...
        occa[:nea] = 1.0
        occb[:neb] = 1.0
        dm = self._scf.make_rdm1((moa, mob), (occa, occb))
        fockao = self._scf.get_fock(dm=dm)
        focka = reduce(np.dot, (moa.conj().T, fockao[0], moa))
        fockb = reduce(np.dot, (mob.conj().T, fockao[1], mob))
        eoa = focka.diagonal().real
        eob = fockb.diagonal().real

        print('focka:\n{}'.format(focka))
        print('fockb:\n{}'.format(fockb))

        print('Fock diagonal (a):\n{}'.format(eoa))
        print('Fock diagonal (b):\n{}'.format(eob))

        eoa = eoa[:nea]
        eob = eob[:neb]

        eri_aa = ao2mo.restore(1, ao2mo.full(self._scf._eri, moa), nmoa)
        eri_bb = ao2mo.restore(1, ao2mo.full(self._scf._eri, mob), nmob)
        eri_ab = ao2mo.general(self._scf._eri, (moa,moa,mob,mob), compact=False)
